package com.zeevpn.android.download.service;
...
import com.zeevpn.android.download.get.DownloadDataSource;
import com.zeevpn.android.download.get.DownloadManager;
import com.zeevpn.android.download.get.DownloadManagerImpl;
import com.zeevpn.android.download.get.DownloadMission;
import com.zeevpn.android.download.get.sqlite.SQLiteDownloadDataSource;
import com.zeevpn.android.download.util.ZeeVpnSettings;
import com.zeevpn.android.models.ZeeVpn;

import java.util.ArrayList;

import static com.zeevpn.android.BuildConfig.DEBUG;
...
public class DownloadManagerService extends Service {
    ...
    public void onCreate() {
        ...
        if (mManager == null) {
            ...
            paths.add(ZeeVpnSettings.getVideoDownloadPath(this));
    ...
    private void startMissionAsync(final String url, final String location, final String name,
                                   final int threads) {
        mHandler.post(() -> {
            int missionId = mManager.startMission(url, location, name, threads);
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        if (action != null && action.equals(Intent.ACTION_RUN)) {
            zeeVpn = intent.getParcelableExtra(EXTRA_NAME);
            String location = intent.getStringExtra(EXTRA_LOCATION);
            int threads = intent.getIntExtra(EXTRA_THREADS, 1);
            String url = intent.getDataString();
            startMissionAsync(url, location, zeeVpn.getFilename(), threads);
        }
...
//            startForeground(NOTIFICATION_ID, mNotification);
    ...
    public static void startMission(Context context, ZeeVpn zeeVpn, String location) {
        Intent intent = new Intent(context, DownloadManagerService.class);
        intent.setAction(Intent.ACTION_RUN);
        intent.putExtra(EXTRA_NAME, zeeVpn);
        intent.putExtra(EXTRA_LOCATION, location);
        intent.putExtra(EXTRA_THREADS, 2);
    ...
    private class MissionListener implements DownloadMission.MissionListener {
        ...
        public void onFinish(DownloadMission downloadMission) {
            ...
            // TODO: 6/5/2018 Send broadcast to activity
        }
